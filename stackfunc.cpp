...
void data_ptr_fix (Stack* stk, int key)
{
    ...
    { 
        printf("INCORRECT DATA PTR FIX KEY, PROGRAMM WILL DO STRANGE THINGS !!!! Farewell to your hard Drive.");
...
void StackDump (Stack* stk, const int str_num, const char* func_name, const char* file_name)
{
    ...
    if (stk)
    {
        ...
        switch (stk->errors)
        {
            case RALLOCATION_ERROR:
                $printf("Error: REALLOCATION RETURN NULL\n");
                break;                
            case STACK_OVERFLOW:
    ...
    PRINT_LINE
    assert(ERROR && "Look at your log file))))))))");
...
int StackOKCheck (Stack* stk)
{
    ...
    int tmp_canary_checker = check_canary(stk);
...
int check_canary (Stack* stk)
{
    ...
    right_data_canary = *((uint64_t*) ((char*) stk->data + stk->capacity * sizeof(elem_t) ) );
